...
# Configuration
MODEL_PATH = r'./models8/checkpoints'
...
class DynamicPINN(nn.Module):
    ...
    def physical_equation(self, original_vw, original_f, original_vs):
        """
...
def physical_constraint(model, inputs, ra, y_scaler, x_scaler):
    ...
    ra_pred = model.physical_equation(
        unscaled_inputs[0],  # original_vw
        unscaled_inputs[1],  # original_f
        unscaled_inputs[2]  # original_vs
    )
...
def analyze_parameter_interactions(model, x_scaler, y_scaler, data, var_names=None, save_dir=None):
    ...
    vw_range = np.linspace(param_ranges['vw']['min'], param_ranges['vw']['max'], n_grid_3d)
    f_range = np.linspace(param_ranges['f']['min'], param_ranges['f']['max'], n_grid_3d)
    vs_range = np.linspace(param_ranges['vs']['min'], param_ranges['vs']['max'], n_grid_3d)

    # Create 3D grid of points
    points = []
    for vw_val in vw_range:
        for f_val in f_range:
            for vs_val in vs_range:
                points.append([vw_val, f_val, vs_val])
    ...
    # Set labels
    ax.set_xlabel('vw (Original Scale)', fontsize=14, fontweight='bold')
    ax.set_ylabel('f (Original Scale)', fontsize=14, fontweight='bold')
    ax.set_zlabel('vs (Original Scale)', fontsize=14, fontweight='bold')

    # Set title
    plt.title('4D Visualization: Surface Roughness (Ra)\nwith respect to vw, f, and vs',
              fontsize=16, fontweight='bold', pad=20)
...
def main():
    ...
    try:
        ...
        data_path = r'C:/pinn/sjzq2000-1.xlsx'
    ...
    X_train_full = train_data.iloc[:, :3].values  # vw, f, vs
    ...
    perform_sensitivity_analysis(
        ...
        var_names=['vw', 'f', 'vs'], save_dir=SENSITIVITY_PATH
    )
    ...
    analyze_parameter_interactions(
        model, X_scaler, y_scaler, full_data,
        var_names=['vw', 'f', 'vs'], save_dir=INTERACTIONS_PATH
    )
